...
/**
 * open_ex_heredoc - open heredoc and any $USER should be exainded
*/

void    open_ex_heredoc(t_minishell *ptr, char **arr, int mode, int i)
{
    char    *file;
    int pid;
    ...
    if (pid == 0)
    {
        ptr->o_file->fd = open(file, O_RDWR | O_CREAT, 0777);
        printf("fd is : %d\n", ptr->o_file->fd);
    }
    free_and_shift(arr, i);
    while (wait(NULL));
    ...
    while (((t_cmd_v1 *)old->content)->cmd[i] && skin->cmd[i][0] != '\0')
    {
        ...
        else if (ft_strncmp(skin->cmd[i], "<<", ft_strlen(skin->cmd[i])) == 0 && skin->flags_red[k++] == 1)
        {
            if (skin->expaind_here[l++] == 1)
                open_ex_heredoc(ptr, skin->cmd, 4, i);
            // else
            //     open_not_ex_heredoc(ptr, skin->cmd, 4, i);
